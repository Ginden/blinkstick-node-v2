...
import { DeviceIdManager } from './id-manager';
...
export abstract class BlinkStick<Transport extends UsbTransport = UsbTransport> {
  ...
  /**
   * Get ID of this specific device.
   * It's stored in infoBlock2, so using this property is incompatible with using infoBlock2 for other purposes.
   */
  get id() {
    return new DeviceIdManager(this);
  }
  ...
  public get [customInspectSymbol]() {
    return {
      $type: this.constructor.name,
      serial: this.serial,
      manufacturer: this.manufacturer,
      product: this.product,
      version: `${this.versionMajor}.${this.versionMinor}`,
